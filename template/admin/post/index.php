            ...
            <?php foreach ($posts as $key => $post) { ?>
                ...
                <tr>
                    ...
                    <td style="width: 10rem;">
                        <?= $post['post_status'] ?>
                        <hr class="my-1">
                        <?php if ($post['post_status'] == 'seen') { ?>
                            <a role="button" class="btn btn-sm btn-success text-white"
                                href="<?= url('admin/post/change-status/' . $post['id']) ?>">click to approved</a>
                        <?php } else if ($post['post_status'] == 'approved') { ?>
                                <a role="button" class="btn btn-sm btn-warning text-black"
                                    href="<?= url('admin/post/change-status/' . $post['id']) ?>">click not to approved</a>
                        <?php } else { ?>
                    ...
                    <td style="width: 25rem;">
                        <a role="button" class="btn btn-sm btn-warning btn-info text-dark"
                        ...
                        <a role="button" class="btn btn-sm btn-warning btn-warning text-dark"
